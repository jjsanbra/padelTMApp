...
public class PlayerRepositoryWithBagRelationshipsImpl implements PlayerRepositoryWithBagRelationships {
    ...
    public Optional<Player> fetchBagRelationships(Optional<Player> player) {
        return player.map(this::fetchCategories);
    }
    ...
    public List<Player> fetchBagRelationships(List<Player> players) {
        return Optional.of(players).map(this::fetchCategories).orElse(Collections.emptyList());
    }

    Player fetchCategories(Player result) {
        return entityManager
            .createQuery("select player from Player player left join fetch player.categories where player.id = :id", Player.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Player> fetchCategories(List<Player> players) {
        ...
        List<Player> result = entityManager
            .createQuery("select player from Player player left join fetch player.categories where player in :players", Player.class)
